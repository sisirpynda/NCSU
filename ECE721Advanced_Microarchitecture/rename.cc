...
void pipeline_t::rename2() {
   ...
   //instantiate svp structures
	// svp svp_obj(0,0,0,PERF_VAL_PRED,this);
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if (eligible(&PAY.buf[index])==true) bundle_vpq++;
      // FIX_ME #1 END
   }
   ...
   // FIX_ME #2 BEGIN
   // cout<<"[RENAME]: bundle_branch:"<<bundle_branch<<" bundle_dst: "<<bundle_dst<<" bundle_vpq: "<<bundle_vpq<<endl;
   // cout<<"[RENAME] branch stall: "<<REN->stall_branch(bundle_branch)<<"reg stall: "<<REN->stall_reg(bundle_dst)<<"vpq stall: "<<SVP->stall_vpq(bundle_vpq)<<endl;
   if (REN->stall_branch(bundle_branch) == true) return; //Condition 1: not enough free checkpoints
   if (SVP->stall_vpq(bundle_vpq) == true && !PERF_VAL_PRED) return; //Condition 1: not enough vpq entries
   ...
   if (eligible(&PAY.buf[index]) ){
      ...
      if(PERF_VAL_PRED ){
         ...
         // Check SVP prediction conditions
            bool in_svp = SVP->is_val_pred_in_svp(PAY.buf[index].pc);
            PAY.buf[index].in_svp = in_svp;
            vpq_index = SVP->create_vpq_entry(PAY.buf[index].pc); // we always create  vpq entry - no gatekeeping
            PAY.buf[index].vpq_index = vpq_index;
         
            if (in_svp) {
               PAY.buf[index].predicted_value = SVP->predict(PAY.buf[index].pc);
               PAY.buf[index].is_confident = SVP->is_confident(PAY.buf[index].pc, 0);
            }
          }
      }

   } else {
      PAY.buf[index].vpq_index = 70000;
      PAY.buf[index].is_confident = false;
      PAY.buf[index].is_predicted = false;
      //SVP->vpmeas_inelegible++;
   }
   ...
   // printf("Predicted value for index %d: %" PRIreg "\n", index, predicted_value);
   

	//Proj4 sis-comment
	//we will inject this into PRF  ---> done in dispatch
      ...
      if (PAY.buf[index].checkpoint == true){
        PAY.buf[index].branch_ID = REN->checkpoint(SVP->tail, SVP->tail_phase);
      }
      // FIX_ME #5 END
   }
