...
void pipeline_t::retire(size_t &instret)
{
    ...
    { // AL head exists and completed
        ...
        if (!exception && !load_viol)
        {
            ...
            // FIX_ME #17b BEGIN
         if (eligible(&PAY.buf[PAY.head])){
               if (!PERF_VAL_PRED)
               { // sis-comment : we do the svp drama when its not perf

                  if (oracleconf)
                  {
                     if (PAY.buf[PAY.head].good_instruction && eligible(&PAY.buf[PAY.head]))
                     { // FIXME: write assertion to check heads of vpq and commiting
                           assert(SVP->vpq_table[SVP->head].pc == PAY.buf[PAY.head].pc);
                           SVP->train_svp();
                     }
                  }
                  else
                  {
                     ...
                     if (eligible(&PAY.buf[PAY.head]))
                     { // in real case you only check if its eligible  //for now we train bullshit and let full squash clean our mess
            ...
            if (eligible(&PAY.buf[PAY.head]) ) {
                ...
                if (PERF_VAL_PRED) {
                    ...
                    if (1)
                        SVP->vpmeas_conf_corr++;
                    if (PAY.buf[PAY.head].is_confident && (PAY.buf[PAY.head].predicted_value != PAY.buf[PAY.head].C_value.dw) && PAY.buf[PAY.head].is_predicted)
                        SVP->vpmeas_conf_incorr++;
                    if (!PAY.buf[PAY.head].is_confident && (PAY.buf[PAY.head].predicted_value == PAY.buf[PAY.head].C_value.dw ) && !PAY.buf[PAY.head].is_predicted)
                        SVP->vpmeas_unconf_corr ++;
                    if (!PAY.buf[PAY.head].is_confident )
                        SVP->vpmeas_unconf_incorr ++;

                }

                if (oracleconf && !PERF_VAL_PRED) {
                    SVP->vpmeas_unconf_corr =0;
                    if (PAY.buf[PAY.head].is_confident && PAY.buf[PAY.head].in_svp)
                        SVP->vpmeas_conf_corr++;
                    if (PAY.buf[PAY.head].is_confident && (PAY.buf[PAY.head].predicted_value != PAY.buf[PAY.head].C_value.dw) && PAY.buf[PAY.head].is_predicted)
                        SVP->vpmeas_conf_incorr++;
                    if (!PAY.buf[PAY.head].is_confident && (PAY.buf[PAY.head].predicted_value == PAY.buf[PAY.head].C_value.dw ) && !PAY.buf[PAY.head].is_predicted)
                ...
                if (!oracleconf && !PERF_VAL_PRED) {
                    if(!PAY.buf[PAY.head].is_confident && !PAY.buf[PAY.head].in_svp)
                        SVP->vpmeas_miss++;
                    if (PAY.buf[PAY.head].is_confident && (PAY.buf[PAY.head].predicted_value == PAY.buf[PAY.head].C_value.dw ))
                        SVP->vpmeas_conf_corr++;
                    if (PAY.buf[PAY.head].is_confident && (PAY.buf[PAY.head].predicted_value != PAY.buf[PAY.head].C_value.dw))
                        SVP->vpmeas_conf_incorr++;
                    if ( !PAY.buf[PAY.head].is_predicted &&  PAY.buf[PAY.head].in_svp && (PAY.buf[PAY.head].predicted_value == PAY.buf[PAY.head].C_value.dw ))
                        SVP->vpmeas_unconf_corr ++;
                    if (!PAY.buf[PAY.head].is_predicted &&  PAY.buf[PAY.head].in_svp && (PAY.buf[PAY.head].predicted_value != PAY.buf[PAY.head].C_value.dw ))
                        SVP->vpmeas_unconf_incorr ++;
                }
            ...
            if (!PAY.buf[PAY.head].split || !PAY.buf[PAY.head].upper)
            {
                ...
                if (num_insn % 1000 == 0)
                {
                  //   SVP->print_svp(stats_log, num_insn);
                  //   SVP->print_vpq(stats_log);
                    //  next_milestone += 1000;
                }
            }
            if (PAY.buf[PAY.head].split && PAY.buf[PAY.head].upper)
