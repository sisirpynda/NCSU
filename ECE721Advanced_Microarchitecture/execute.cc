       ...
       if (Execution_Lanes[lane_number].ex[depth].valid)
       {
           ...
           if (IS_MEM_OP(PAY.buf[index].flags))
           {
               ...
               if (IS_LOAD(PAY.buf[index].flags))
               {
                   ...
                   // FIX_ME #13 BEGIN ///added
                   if (PAY.buf[index].C_valid && hit) // add flag check before wakeup)
                     {
                        if (eligible(&PAY.buf[index]))
                        {
                              if (!PERF_VAL_PRED)
                              {
                                 SVP->update_vpq_entry(PAY.buf[index].vpq_index, PAY.buf[index].C_value.dw);
                                 ...
                                 else if (!PAY.buf[index].is_predicted)
                                 {
                  ...
                  //  if (PAY.buf[index].C_valid && hit) // add flag check before wakeup)
                  //  {
                  //      if (PAY.buf[index].is_predicted == false)
                  //      {
                  //          IQ.wakeup(PAY.buf[index].C_phys_reg);
                  //          REN->set_ready(PAY.buf[index].C_phys_reg);
                  //          REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].C_value.dw);
                  ...
                  //  }
                   ...
                   // if(!PERF_VAL_PRED && (PAY.buf[index].is_predicted == true) && hit){
                   //    //update vpq
                   //       // SVP->update_vpq_entry(PAY.buf[index].vpq_index , PAY.buf[index].C_value.dw) ;
                   //       assert(PAY.buf[index].C_valid);
                   //       // PAY.buf[index].execute_value.dw=PAY.buf[index].C_value.dw;
                   // }
           ...
           else
           {
            ...
            if (PAY.buf[index].C_valid) // add flag check before wakeup)
            {
                if (eligible(&PAY.buf[index]))
                {
                    if (!PERF_VAL_PRED)
       ...
       if (depth > 0)
       {
           ...
           if (Execution_Lanes[lane_number].ex[depth - 1].valid)
           {
               ...
               if ((PAY.buf[index].C_valid == true) &&
                   ...
                   (IS_AMO(PAY.buf[index].flags) == false) // add flag check before wakeup
               )
               {
                   ...
                   // else if(!PERF_VAL_PRED && (PAY.buf[index].is_predicted == true)){
                   //    //update vpq
                   //       // SVP->update_vpq_entry(PAY.buf[index].vpq_index , PAY.buf[index].C_value.dw) ;
                   //       assert(PAY.buf[index].C_valid);
                   //       PAY.buf[index].execute_value.dw=PAY.buf[index].C_value.dw;
               ...
               //    if(!PERF_VAL_PRED){
               //       //update vpq
               //          SVP->update_vpq_entry(PAY.buf[index].vpq_index , PAY.buf[index].C_phys_reg) ;
               //       }
   ...
   void pipeline_t::load_replay()
   {
       ...
       if (LSU.load_unstall(cycle, index, value))
       {
           ...
           if (PAY.buf[index].C_valid) // add flag check before wakeup
           {
               ...
               // if(!PERF_VAL_PRED && (PAY.buf[index].is_predicted == true)){
               //    //update vpq
               //       // SVP->update_vpq_entry(PAY.buf[index].vpq_index , PAY.buf[index].C_value.dw) ;
               //       assert(PAY.buf[index].C_valid);
               //       PAY.buf[index].execute_value.dw=PAY.buf[index].C_value.dw;
