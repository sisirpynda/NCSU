...
void pipeline_t::dispatch() {
   ...
   // cout<<"[DISPACH]: bundle_inst:"<<bundle_inst<<" bundle_load: "<<bundle_load<<" bundle_store: "<<bundle_store<<endl;
   // cout<<"[DISPATCH] iq stall: "<<IQ.stall(bundle_inst)<<"lsu stall: "<<LSU.stall(bundle_load,bundle_store)<<endl;
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      //PAY.buf[index].AL_index = REN->dispatch_inst(PAY.buf[index].C_valid, PAY.buf[index].C_log_reg, PAY.buf[index].C_phys_reg, IS_LOAD(PAY.buf[index].flags),store_flag, IS_BRANCH(PAY.buf[index].flags), IS_AMO(PAY.buf[index].flags), IS_CSR(PAY.buf[index].flags), PAY.buf[index].pc );

            bool dest_valid = PAY.buf[index].C_valid;
      load_flag = IS_LOAD(PAY.buf[index].flags);
      ...
      PAY.buf[index].AL_index = REN->dispatch_inst(dest_valid, PAY.buf[index].C_log_reg, PAY.buf[index].C_phys_reg, load_flag, store_flag, branch_flag, amo_flag, csr_flag, PAY.buf[index].pc
      );
      // FIX_ME #7 END
      ...
      // FIX_ME #8 BEGIN
      	// the ready bits come from rename class
...
//		D_ready = 1;
       ...
       if (PAY.buf[index].A_valid == true){A_ready = REN->is_ready(PAY.buf[index].A_phys_reg);}
      else {A_ready = true;}
      if (PAY.buf[index].B_valid == true){B_ready = REN->is_ready(PAY.buf[index].B_phys_reg);}
      else {B_ready = true;}
      if (PAY.buf[index].D_valid == true){D_ready = REN->is_ready(PAY.buf[index].D_phys_reg);}
      else {D_ready = true;}
      ...
      // FIX_ME #9 BEGIN
       //if (PAY.buf[index].A_valid)
	//	REN->clear_ready(PAY.buf[index].A_phys_reg);
	//if (PAY.buf[index].B_valid)
	//	REN->clear_ready(PAY.buf[index].B_phys_reg);
	//if (PAY.buf[index].D_valid)
	//	REN->clear_ready(PAY.buf[index].D_phys_reg);
	//if (PAY.buf[index].C_valid)
	//	REN->clear_ready(PAY.buf[index].C_phys_reg);

      
      if (PAY.buf[index].C_valid == true) REN->clear_ready(PAY.buf[index].C_phys_reg);

      PAY.buf[index].is_predicted = false;
      ...
      if ( eligible(&PAY.buf[index])) {
         ...
         if(PERF_VAL_PRED){
            if(PAY.buf[index].good_instruction){
               ...
               PAY.buf[index].predicted_value = actual_value;
               REN->write(PAY.buf[index].C_phys_reg , PAY.buf[index].predicted_value );
               REN->set_ready(PAY.buf[index].C_phys_reg);   
               PAY.buf[index].is_predicted = true;
               PAY.buf[index].is_confident = true;
            }
         }
         else{
            if(oracleconf){
               bool in_svp = SVP->is_val_pred_in_svp(PAY.buf[index].pc);
               if(in_svp && PAY.buf[index].good_instruction){
                  
                  db_t* actual = get_pipe()->peek(PAY.buf[index].db_index);
                  int64_t actual_value = actual->a_rdst[0].value;
                  
               int64_t predicted_value = SVP->predict(PAY.buf[index].pc);
               PAY.buf[index].predicted_value = predicted_value;
                  PAY.buf[index].is_confident = SVP->is_confident(PAY.buf[index].pc, actual_value );
               ...
               bool in_svp = SVP->is_val_pred_in_svp(PAY.buf[index].pc);
               if(in_svp && PAY.buf[index].is_confident){
                  //int64_t actual_value = PAY.buf[index].predicted_value;
                  REN->write(PAY.buf[index].C_phys_reg , PAY.buf[index].predicted_value);
                  REN->set_ready(PAY.buf[index].C_phys_reg); 
                  PAY.buf[index].is_predicted = true;
               }
      ...
      // uint64_t vpq_index = 404;

      // if (PAY.buf[index].good_instruction && eligible(&PAY.buf[index]) ) {
      
      //     if(!PERF_VAL_PRED) {
      //       vpq_index = SVP->create_vpq_entry(PAY.buf[index].pc);
      //       PAY.buf[index].vpq_index = vpq_index;
      //    }
	...
	//	D_ready = REN->is_ready(PAY.buf[index].D_phys_reg);
      ...
      //    * Dispatch the instrction into the Issue Queue.
