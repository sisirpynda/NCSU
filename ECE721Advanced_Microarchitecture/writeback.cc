...
void pipeline_t::writeback(unsigned int lane_number) {
   ...
   if (Execution_Lanes[lane_number].wb.valid) {
      ...
      if (PAY.buf[index].checkpoint) {
		 ...
		 //sis uncomment assert statement
		 ...
		 REN->resolve(PAY.buf[index].AL_index, PAY.buf[index].branch_ID, 1);
		 ...
		 REN->resolve(PAY.buf[index].AL_index, PAY.buf[index].branch_ID, 1);
		 resolve(PAY.buf[index].branch_ID, 1);
            // FIX_ME #15b END
         }
         else {
            ...
            SVP->vpq_partial_rollback(REN->branch_checkpoints[PAY.buf[index].branch_ID].vpq_tail_cp, REN->branch_checkpoints[PAY.buf[index].branch_ID].vpq_phase_cp);
	    
	         REN->resolve(PAY.buf[index].AL_index, PAY.buf[index].branch_ID, 0);

      // for(int i=0; i< 1024; i++){
         //if()
         //if( (SVP->svp_table[i].valid) && (REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].tag == SVP->svp_table[i].tag) ){ 
         //SVP->svp_table[i].instance = REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].instance;
         //SVP->svp_table[i].stride = REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].stride;
         //SVP->svp_table[i].conf = REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].conf;
         //SVP->svp_table[i].retired_value = REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].retired_value;
         //}
     ...
     //  for(int i=0; i< 1024; i++){
      //   if( (REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].valid && SVP->svp_table[i].valid) && (REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].tag == SVP->svp_table[i].tag) ){ 
      //   assert(REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].tag == SVP->svp_table[i].tag);
         // assert(REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].conf == SVP->svp_table[i].conf);
         // assert(REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].retired_value == SVP->svp_table[i].retired_value);
         // assert(REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].stride == SVP->svp_table[i].stride);
         // assert(REN->branch_checkpoints[PAY.buf[index].branch_ID].svp_table_cp[i].instance == SVP->svp_table[i].instance);
        // }
            ...
            // FIX_ME #15d BEGIN
		      resolve(PAY.buf[index].branch_ID, 0);		// reason why its done after LSU restore ???
            // FIX_ME #15d END
      ...
      // if(!PERF_VAL_PRED && eligible(&PAY.buf[index])) {
      //    //update vpq
      //       SVP->update_vpq_entry(PAY.buf[index].vpq_index , (int64_t)PAY.buf[index].C_value.dw);
      //       if(!oracleconf && PAY.buf[index].is_predicted){
      //          if(PAY.buf[index].predicted_value != PAY.buf[index].C_value.dw){
      ...
      // detect if prediction is clapped
