...
// class svp;  // Forward declaration (add near other friend declarations)
...
//sisir added eligibility code 
//KS: Fixed it

inline bool eligible(payload_t *pay) {
	if (!pay->C_valid)
		return(false); // Any instruction without a destination register is inelegible.
