...
void lsu::execute_load(cycle_t cycle,
   ...
   if (LQ[lq_index].amo) { // Load reservation.
      if ((lq_index == lq_head) && (lq_index_phase == lq_head_phase)) {
         // Load reservation has reached the head of the LQ.
         // It will be executed when being committed at the retire stage.
      }
      else {
...
void lsu::commit(bool load, bool atomic_op) {
   if (load) {
      // LQ should not be empty.
      assert(lq_length > 0);

      // If this is a load-reserved (serialized), set the load_reservation state
      // for the following store-conditional to verify the atomicity.
      if (atomic_op) {
         proc->get_state()->load_reservation = LQ[lq_head].addr;
      }
      ...
      if (atomic_op) {
         send_to_memory = (proc->get_state()->load_reservation == SQ[sq_head].addr);

         if (proc->PAY.buf[SQ[sq_head].pay_index].C_valid) {
            assert(proc->PAY.buf[SQ[sq_head].pay_index].C_log_reg != 0); // If X0, would have cleared C_valid in Decode Stage
            reg_t sc_result = (send_to_memory ? 0 : 1);                  // SC writes zero to rd on success or a nonzero code on failure.
            proc->PAY.buf[SQ[sq_head].pay_index].C_value.dw = sc_result;
            proc->REN->set_ready(proc->PAY.buf[SQ[sq_head].pay_index].C_phys_reg);
            proc->REN->write(proc->PAY.buf[SQ[sq_head].pay_index].C_phys_reg, sc_result);
         }
      }
      else {
         send_to_memory = true;
