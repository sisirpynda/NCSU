...
#undef OPEN_LOG_FILE
   ...
   /////////////////////////////////////////////////////////////
   // SVP module instantiation 
   /////////////////////////////////////////////////////////////
   //instantiate svp structures
   // SVP = new svp(VPQsize,index_bits,tag_bits,confmax,PERF_VAL_PRED); //dw 'this' still points to the pipeline 

   cout << "[pipeline] : Constructor being called with parameters:" << endl;
   ...
   SVP = new svp(VPQsize, index_bits, tag_bits, confmax, PERF_VAL_PRED, oracleconf);
   ...
   fprintf(stats_log, "ENABLE_TRACE_CACHE = %d\n", (ENABLE_TRACE_CACHE ? 1 : 0));
      ...
      //SVP->print_val_meas(stats_log);
fprintf(stats_log, "=== VALUE PREDICTOR ============================================================\n\n");

fprintf(stats_log, "VP-eligible configuration:\n");
fprintf(stats_log, "   predINTALU = %lu\n", predINTALU);
fprintf(stats_log, "   predFPALU  = %lu\n", predFPALU);
fprintf(stats_log, "   predLOAD   = %lu\n\n", predLOAD);

if (!PERF_VAL_PRED) {
   fprintf(stats_log, "VALUE PREDICTOR = stride (Project 4 spec. implementation)\n");
   fprintf(stats_log, "   VPQsize         = %lu\n", VPQsize);
   fprintf(stats_log, "   oracleconf      = %lu\n", oracleconf);
   fprintf(stats_log, "   # index bits    = %lu\n", index_bits);
   fprintf(stats_log, "   # tag bits      = %lu\n", tag_bits);
   fprintf(stats_log, "   confmax         = %lu\n\n", confmax);
} else {
...
// Calculate SVP storage costs
uint64_t conf_bits = static_cast<uint64_t>(ceil(log2(confmax + 1)));
uint64_t instance_ctr_bits = 0;
if(VPQsize != 0)
instance_ctr_bits = static_cast<uint64_t>(ceil(log2(VPQsize)));
int bits_per_entry = tag_bits + conf_bits + 64 + 64 + instance_ctr_bits;
uint64_t total_bits = (1<<index_bits) * bits_per_entry;
double total_bytes = static_cast<double>(total_bits) / 8.0;

if ( !(VPQsize == 0 || index_bits == 0) ) {

fprintf(stats_log, "      tag           : %3lu bits  // num_tag_bits\n", tag_bits);
...
fprintf(stats_log, "      -------------------------\n");
fprintf(stats_log, "      bits/SVP entry: %d bits\n", bits_per_entry);
fprintf(stats_log, "   Total storage cost (bits) = (%d SVP entries x %d bits/SVP entry) = %lu bits\n",
(1<<index_bits), bits_per_entry, total_bits);
...
bool pipeline_t::step_micro(size_t instret_limit, size_t &instret) {
   ...
   static uint64_t next_milestone = 1000;

   // if (num_insn >= next_milestone && next_milestone <= 20000) {
   //    SVP->print_svp(stats_log);
   //    next_milestone += 1000;
   // }
