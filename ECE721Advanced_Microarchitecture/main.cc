...
//KS: changed scanf to !=3 instead of !=4.
////////////////////PROJ4////////////
static void set_vp_flags(const char *config) {
   uint64_t pialu, pfpalu, pload;
   if (sscanf(config, "%lu,%lu,%lu", &pialu, &pfpalu, &pload) != 3) {
      fprintf(stderr, "Incorrect usage of --vp-eligible=<pialu>,<pfpalu>,<pload>\n");
      fprintf(stderr, "...where vp-eligible (value prediction eligible instructions), pialu (predict int typ instr), piflalu (predict fp typ instr), and pload (predict load typ instr) are each 0 or 1.\n");
      exit(-1);
   }
   else {
      predINTALU = (pialu ? true : false);
      predFPALU = (pfpalu ? true : false);
      predLOAD = (pload ? true : false);
   }
}
static void set_svp_inputs(const char *config) {

   uint64_t vpq_size, oracle_conf, num_svp_index_bits, num_svp_tag_bits, conf;
   
   if (sscanf(config, "%lu,%lu,%lu,%lu,%lu", 
             &vpq_size, &oracle_conf, &num_svp_index_bits, &num_svp_tag_bits, &conf) != 5) {
      fprintf(stderr, "Incorrect usage of --vp-svp=<vpq_size>,<oracleconf>,<index_bits>,<tag_bits>,<confmax>\n");
      fprintf(stderr, "Expected 5 comma-separated values:\n");
      fprintf(stderr, "1. vpq_size: VPQ entries (e.g., 64)\n");
      fprintf(stderr, "2. oracleconf: Oracle confidence threshold (0-3)\n");
      fprintf(stderr, "3. index_bits: Number of SVP index bits (e.g., 8)\n");
      fprintf(stderr, "4. tag_bits: Number of SVP tag bits (e.g., 12)\n");
      fprintf(stderr, "5. confmax: Maximum confidence counter value (e.g., 3)\n");
      exit(-1);
   }
   else {

      VPQsize = vpq_size;
      oracleconf = oracle_conf;  // Assuming these are class/struct members
      index_bits = num_svp_index_bits;
      tag_bits = num_svp_tag_bits;
      confmax = conf;

      cout << "[main] : parameters:" << endl;
      cout << "VPQsize: " << VPQsize << endl;
      cout << "oracleconf: " << oracleconf << endl;
      cout << "index_bits: " << index_bits << endl;
      cout << "tag_bits: " << tag_bits << endl;
      cout << "confmax: " << confmax << endl;
...
int main(int argc, char **argv) {
   ...
   parser.option('L', 0, 0, [&](const char *s) {
      ...
      // sisir added options extension - sis 
      //KS:updated
	parser.option(0, "vp-eligible", 1, [&](const char *s) { set_vp_flags(s);}) ;
   parser.option(0, "vp-perf", 1, [&](const char *s) {
      PERF_VAL_PRED = atoi(s);
      if (PERF_VAL_PRED) {
          fprintf(stderr, "\n\n\tPROJ4-DEBUG : PERF_VAL_PRED enabled\n\n");
      }
  });

//   set_svp_inputs
parser.option(0, "vp-svp", 1, [&](const char *s) { set_svp_inputs(s);}) ;
