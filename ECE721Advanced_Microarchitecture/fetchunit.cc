...
fetchunit_t::fetchunit_t(uint64_t instr_per_cycle,                      // "n"
                         ...
                         uint64_t cbp_algorithm,                        // conditional branch predictor algorithm: 0 for gshare, 1 for tage-sc-l
                         ...
                         ras_recover_e ras_recover_model,               // RAS recovery model
   ...
   // Create the branch predictors.  This approach may feature polymorphism.
   switch (cbp_algorithm) {
   case 0: // gshare
      CBP = new gshare_t(true, cond_branch_per_cycle, cb_pc_length, cb_bhr_length, bq_size);
      break;

   case 1: // tage-sc-l
      assert(cond_branch_per_cycle == 1);
      CBP = new tagescl_wrapper_t(bq_size);
      break;
   ...
   IBP = new gshare_t(false, 1, ib_pc_length, ib_bhr_length, bq_size);
   RBP = new ras_t(ras_size, ras_recover_model, bq_size);
...
void fetchunit_t::spec_update(spec_update_t *update, uint64_t cb_predictions) {
   // Speculatively update the predictors' contexts.
   CBP->spec_update(cb_predictions, update->num_cb, pc, update->next_pc, update->pop_ras, update->push_ras, update->push_ras_pc);
   IBP->spec_update(cb_predictions, update->num_cb, pc, update->next_pc, update->pop_ras, update->push_ras, update->push_ras_pc);
   RBP->spec_update(cb_predictions, update->num_cb, pc, update->next_pc, update->pop_ras, update->push_ras, update->push_ras_pc);
...
void fetchunit_t::fetch1(cycle_t cycle) {
   ...
   if (bp_perfect) {
      ...
      // "m" taken/not-taken predictions are packed into a uint64_t.
      cb_predictions = CBP->predict(pc);
      ...
      ib_predicted_target = IBP->predict(pc);

      // Get a predicted target from the return branch predictor.  It is only used if the fetch bundle ends at a return instruction.
      ras_predicted_target = RBP->predict(pc);
   }
   ...
   if (tc_hit || !ic_miss) {
      ///////////////////////////////////////////////////////////////////////////////////////////////////////////
      // Save the fetch bundle's pc, PAY's current position, and T$ hit info in the fetch2_status register.
      // Likewise, signal all branch predictors to save their contexts (prior to the fetch bundle).
      ...
      CBP->save_fetch2_context();
      IBP->save_fetch2_context();
      RBP->save_fetch2_context();
      ...
      ///////////////////////////////////////////////////////////////////////////////////////////////////////////
      // Speculatively update the pc and all branch predictors' contexts.
      ///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool fetchunit_t::fetch2(pipeline_register DECODE[]) {
   ...
   if (misfetch) {
      ...
      // c. Rollback the Fetch1 stage to what its state was just prior to the misfetched bundle -- in order to repredict it.
      pc = fetch2_status.pc;
      CBP->restore_fetch2_context();
      IBP->restore_fetch2_context();
      RBP->restore_fetch2_context();
...
void fetchunit_t::mispredict(uint64_t branch_pred_tag, bool taken, uint64_t next_pc) {
   ...
   // 3. Restore predictors' contexts.

   CBP->mispredict(pred_tag, (bq.bq[pred_tag].branch_type == BTB_BRANCH), taken, next_pc);
   IBP->mispredict(pred_tag, (bq.bq[pred_tag].branch_type == BTB_BRANCH), taken, next_pc);
   RBP->mispredict(pred_tag, (bq.bq[pred_tag].branch_type == BTB_BRANCH), taken, next_pc);
...
void fetchunit_t::commit(uint64_t branch_pred_tag) {
   ...
   case BTB_BRANCH:
      CBP->commit(pred_tag,
                  bq.bq[pred_tag].fetch_pc,
                  bq.bq[pred_tag].fetch_cbID_in_bundle,
                  bq.bq[pred_tag].taken,
                  bq.bq[pred_tag].next_pc /* dont-care */);
   ...
   case BTB_CALL_INDIRECT:
      IBP->commit(pred_tag,
                  bq.bq[pred_tag].fetch_pc,
                  bq.bq[pred_tag].fetch_cbID_in_bundle /* dont-care */,
                  bq.bq[pred_tag].taken /* dont-care */,
                  bq.bq[pred_tag].next_pc);
...
void fetchunit_t::flush(uint64_t pc) {
   // Only do steps 1 and 2 if the branch queue isn't empty.
   if (!bq.empty()) {
      // 1. Roll-back the branch queue to the head entry.
      uint64_t pred_tag = bq.flush(); // "pred_tag" is the index of the head entry.

      // 2. Restore predictors' contexts.
      CBP->flush(pred_tag);
      IBP->flush(pred_tag);
      RBP->flush(pred_tag);
   }
